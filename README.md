...
已提供 `check-deps` 子命令：仅检查 `Contents` 下普通文件中识别出的 64 位 Mach-O（含 fat 切片，不跟随符号链接），在 stdout 输出单个 JSON 报告（`app`、`root`、`bins`、`totalChecked`、`totalMissing`）。CPU 名映射：cputype 0x01000007→arm64、0x0100000c→x86_64，其他切片用 `0x` 加八位小写十六进制。`libs` 只含 LC_LOAD_DYLIB、LC_LOAD_WEAK_DYLIB、LC_REEXPORT_DYLIB 的路径，剔除 `/usr/lib/`、`/System/Library/` 字面前缀（区分大小写），同一切片按路径升序去重。未指定 `--require-arch` 时每文件一条记录（fat 文件 `arch` 为 null，`libs` 为各切片合并去重升序）；指定时每切片一条（`arch` 为切片 CPU 名），Mach-O 缺该架构切片时追加带 `missing` 字段的记录。`bins` 按 `path` 升序、同 `path` 按 `arch` 升序（null 最小）。退出码：无缺失（或未指定 `--require-arch`）0；有 missing 为 1；无 Mach-O 为 4；`--require-arch` 非法、路径不存在或非 .app 为 2；缺 Contents 或不可读为 3。

无参数显示帮助，未知参数以非零状态退出。尚未实现信任检查、发布比较以及更新渠道检查，不会创建业务数据文件。
